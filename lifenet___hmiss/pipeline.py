...
def fetch_data_value_sets239(data_element_ids: List[str],
                         periods: List[str],
    ...
    try:
        ...
        if periods:
            params["period"] = ";".join(periods)  # DHIS2 2.39 uses semicolon for multiple periods
        if org_units:
            params["orgUnit"] = ";".join(org_units)  # DHIS2 2.39 uses semicolon for multiple org units
        if data_element_ids:
            params["dataElement"] = ";".join(data_element_ids)  # DHIS2 2.39 uses semicolon for multiple data elements
...
def get_data(
    ...
    try:
        ...
        if conn_identifier == "uganda-hmis":
            data = fetch_data_value_sets239(data_elements, periods, org_units, conn_identifier, start_date)
        else :
